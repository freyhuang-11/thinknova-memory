...
metadata:
  ...
  modified: 2026-09-13T14:09:04.594Z
---
...
**文档位置**:微信 `D:\xwechat_files\wxid_5aowhntcmaoh12_6db1\msg\file\2026-0N\`(按月);本地归档 `00_规格与参考\技术侧文档\`(老板微信收的技术文档;老板每次发新的当天归档,见 [[reference-prompt-library]])。

🔴🔴 **合订本(07-30 老板令建,日常第一翻阅点)**:`D:\SamsoData\Documents\视频制作平台分析\00_规格与参考\技术手册_合订本_现行版.md` —— 全部技术文档的现行口径按主题归并,文末带「已作废口径」清单。**收到新文档→当天并入合订本+覆盖记忆**。权威序:原文档>合订本>记忆。

## 🔴 口径速查(最常被我记错的几条,2026-07-29 逐字回源核过)
1. **`i2vReferenceStrategy` 默认值 = `panel_crop`**,文档原话「**这是默认且推荐的方式**」(手册 v4 §3.5)。`storyboard_board` 有硬门槛:「**只建议对已验证不会把网格生成进成片的模型使用**」。→ grok 415(无网格率仅 ~75%)**不该用整板**;omni 460(100% 零网格)可以。
   ✅ **线上已对齐**:2026-07-29 03:43:45 全局由 `storyboard_board` 翻成 `panel_crop`(`masterPipeline` + `opsEditable` 双写),**偏离项已消除**,全库残留 `storyboard_board` 0。
...
4. 🔴 **文档说"运营可维护"≠ 我改得进去**:`businessUi.*`(detailOptionGroups/videoGeneration/referenceCases…)虽在官方运营可维护清单里,但**后台弹窗 textarea 对它的修改被静默丢弃且回执报成功**(07-29 实证,字节账见 [[reference-thinknova-config-powers]] 顶部)。要改只能人工点界面或走案例库单条 PUT 路由。
5. **`lineValidation` 是按语言的平表、没有时长维度**(11 键):8/10/12/15 秒共用一个 min–max → 15 秒片子写 45 字照样过校验。**加时长维度=改结构=交技术**,文档里也没有这个能力。
...
| **编剧多语言与回退配置** | 07-19 | 多语言字数+回退 | 路径`promptComposer.masterPipeline.scriptwriter`;`lineValidation`+`fallbackPolicy`(mode=ambient_only无口播/allowCta=false);清理旧"只允许中文/60-75"硬编码。⚠️ **文档里的 zh 60-75 / en 25-40 是当时示例,已被运营改过**;线上现值(07-29)=`zh{45,100}` `en{25,50}` `ja{55,95}` `ko{55,85}`,`zh_en` 无键落 default。**数值一律以线上回读为准** |
...
| 🆕🆕🆕**配置JSON说明书(3)** | **07-30 收到**(文件标07-14,内容为最新修订版,**与旧版冲突以本版为准**) | config_json 全结构最新权威(海报+视频) | 🔴🔴 **`copyLanguage.optionsMode:"replace"`**=语言整表替换,删的不补回;配套 `allowEmpty/emptyLabel/betaLabel/options[].beta`;启用语言必须在 `promptComposer.languagePolicy.map` 有指令(07-30 已用:前台 19→6 语言)。<br>🔴🔴 **`scriptwriter.textModel.temperature` = 编剧多样性核心**,默认0.2=同场景每次几乎一样,文档建议0.6-0.8(07-30 已设0.7双写)。<br>🔴🔴 **编剧 systemPrompt 四级回退链**:`masterPipeline.scriptwriter.systemPrompt`(最高) > `screenwriter.systemPrompt`(旧兼容,**线上 masterPipeline 层为空所以我一直改的这层有效**) > `screenwriterPrompt` > 代码内置;staticTemplates 同理三级。**以后新改动优先写 masterPipeline 层。**<br>🔴 **多参提交规则**:商家工作流,模型 maxRef=1 → 只交裁格首帧;**≥2 → 只交裁格首帧+完整分镜板共2张**(不加用户图!与07-28文档"按剩余名额加用户上传图"不同,**以本版为准**)。<br>🔴 「30秒只保留历史任务兼容,不应再作新任务配置」(文档口径;老板07-30说明天上30秒——上线时以技术实际发版为准)。<br>`businessUi.defaultImageModelId/defaultVideoModelId` 可选,ID须当前环境;视频模型优先级=用户选择>Agent默认>全局。`videoModelPolicy` 在 promptAssembler 下。`assetVisibilityRules/assetFilterRules` 管中间产物可见性。`imageEditOptions.video_first_frame.strength=0.74`。场景九~十二=多样性/字幕/6格/禁文字道具的官方修法。 |
...
- 2026-09-13 《运营说明_Studio多模态素材分析与文本编剧分层_2026-09-13.md》— studioWorkflow.referenceAnalysis{enabled,modelId,prompt,reuseEnabled}/迁移 085/pricing.referenceAnalysis;代码完成未上线 → 归档 `00_规格与参考\技术侧文档\`;消化件 `03_工作台\技术新文档三份_消化与记忆冲突_2026-09-13.md`
- 2026-09-13 《运营说明_旧商家视频素材分析与文本编剧_2026-09-13.md》— promptComposer.masterPipeline.materialAnalysis{enabled,modelId,prompt,reuseEnabled,failureMode}/internal_child;未上线 → 归档 `00_规格与参考\技术侧文档\`
