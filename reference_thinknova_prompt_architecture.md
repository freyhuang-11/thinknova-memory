...
## ~~六宫格描述法~~(已废,2026-07-19 老板拍板重构)
旧法"输入含六宫格分镜板按6格顺序演"已整体废除——grok1.5 只收1张裁格首帧(max_reference_images=1),板到不了模型,格位语言=喂给模型一个不存在的东西还得靠负面词解毒。**现行=按秒镜头清单**:videoTemplate 改"由多个按时间顺序推进的镜头组成,输入图片=第1镜首帧",storyboardGuard 改事实版("输入仅一张首帧图");生图侧六宫格+裁格流程保留不动(为将来换模型留架构)。

## 🔴 2026-07-19/20 大改造(现行状态,动提示词前以此为准)
- **台词长度**:代码真校验=lineValidation(masterPipeline.scriptwriter,11语言分口径:zh系60-75字符含标点去空格/en等25-40词/ja·ko 55-85);systemPrompt 已删全部硬编码口径,改"唯一准绳=输入JSON的 lineLengthTarget(metric/min/max)+copyLanguage"。回退策略=fallbackPolicy ambient_only(无口播纯环境音,allowCta:false)。
- **编剧新铁律**:台词画面同频(台词只说该格画面里的事)/画面细节三要素(商品本体在场+进行中动作+微距点名部位)/【长度永远优先】(新规则不许顶破字数)/行业模板仅供风格(主体=商品名,品类不符禁灌行业典型物)/结尾句纪律(CTA空=零邀请,邀请只许第五句)/禁词自检(CTA空时中英禁词清单机械查——**临时措施**,见下)/【译名例外】(copyLanguage与商品名语言不同用译名,台词语言纯净)。
- **🔴到店标准(老板2026-07-20拍板)**:提示词对"到店"必须**沉默**(不教不禁不提);自然偶发一句=正常创作;**每条必现=提示词在灌=错**。禁词自检是验证期临时手段,证明灌输源断净后要撤掉恢复沉默态。到店灌输源四层清除史:①删【召唤只软性】示例②13场景规则句4拆改③26行业"召唤="36处清零④案例endingCtaPreset全清(987条)。
- **色彩**:全链路去高饱和(首帧色泽铁律/videoTemplate/3行业调色)="原相机直出:饱和克制肤色准确白平衡正常,质感靠光影层次;绝不高饱和滤镜感也绝不灰暗"。
- **复读=废片**:每句只说一次,说完宁留环境音绝不重复(治回退单"到店看看"循环+正常单填时长复读)。
- **编剧模型=gpt-5.6-luna(461)现役**(老板拍板,deepseek是老的不切回)。卖点预设全量裁1。
- **字节**:i2v 4096 上限仍在,videoTemplate 每改必配等量瘦身,改后烧单量真实 Blob size。
...
## 2026-07-20 晚·先保成功率(老板定调,推翻多条旧结论)

**老板原话**:"很多规则实施不了是常态,grok 就是不能硬切,我们要保证至少 90% 以上的成功率,才是效果的事"。
→ **优先级铁律:先保出片率,再谈效果。grok 做不到的规则写了也不执行,纯占字节,一律不写。**

**1. 4096 字节截断雷(静默,不报错)**
- i2v 提交串超 4096 字节被静默砍尾,**镜头和收尾规则消失但任务仍显示 succeeded**;
- 实测:一单 4093 字节 → 五镜头只剩 2 个 + 收尾段全丢;同期多单 4070/4024/3999 全贴边;
- 成因链:给编剧加规则 → cells 写得更细更长 → 间接吃爆 i2v 字节;
- **改任何提示词后必须量提交串字节 + 数【子格】次数 + 确认收尾段还在**,不能只看配置写入。

**2. videoTemplate 瘦身 827→436 字(现行值)**
- 砍掉(实拍证明不执行):一镜一换硬切铁律、景别跨档、角度30度、不越轴、cut on action、no crossfade/dissolve/morph、纯口播一镜到底、逐镜音效、口播语气细节;
- 保留(决定成败):单一全屏绝不分屏拼贴、人物以首帧为准(首帧无人则全片不添人)、台词每句只说一次+不加速吞字+数字汉字读、同店同光同人+肤质真实+原相机直出+零文字水印。

**3. 六宫格整板实测结论(推翻裁格的老依据)**
- **旧结论"整板会被当一整张图一起动"在有黑屏锚后已不成立**:整板直接喂,15 秒按子格2→6 推进,景别跨档、同鞋同店同光一致性好;
- 唯一代价:**开头 0.25 秒漏一帧完整六宫格**(黑屏锚挡不住),合成层切 0.3 秒即可;
- 收益:镜头数 1→5、无放大无变形(单格 480x1280=3:8 需拉伸,整板 1440x2560=9:16 完美匹配);
- **grok 的切换是叠化不是硬切**(切点有半透明重影帧)——这是模型能力边界,不是提示词问题;
- 取消裁格需技术改代码,待老板拍板。

**4. 人脸镜头配额(压审核的现役手段,待验证)**
- 规则:五镜头里至少两格完全无人脸,有人脸最多三格,绝不连续三格同一张脸近景正脸;
- 依据:对照实验 product_only 过 / owner_speaking 拒;无人脸镜头≥2 格的单全过、仅 1 格的全拒;
- **尚未得到干净验证**(首次验证单因截断作废)。
