...
- **当前配方=E14版·白边单帧架构(母版候选v2,已在线上;等用户确认后连跑3-4条稳定性)**:**放弃分镜板**——首帧=中央9:16竖构图真实第一帧+左右白边,三行旁白写白边上(口播能读、成片裁掉白边文字永不入画);后续拍子(过程/成品/收尾微笑)由i2v文本驱动。实测:成片=首帧1:1延续、尾帧吃到(9.5s抬头看镜)、旁白无语气词三句全念。机理:grok衍生不可对抗只能顺应,视觉→首帧/时间轴→i2v文本/台词→裁切区文字。台词规则:无呀哦语气词、情绪靠内容节奏、不机器人生硬;眼神有焦点不呆滞。分镜板路线留给seedance复用。
