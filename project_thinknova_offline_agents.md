...
## 🔴 当前焦点(压缩后先看这里)
1. **P1 六宫格成片**(2026-07-13):成片=整块六宫格分镜板在动。真根因=裁格前 PHP cURL 从 `cos.lingkeai.vip` 下载分镜板时 SSL 证书链校验失败(result:19=链含自签/取不到签发者)→裁切失败(`OFFLINE_STORE_MASTER_PANEL_CROP_FAILED`)→旧逻辑兜底把整板发 i2v。实证 `task_a7d925084fa3`(整板既当 `input.image_url` 又当 `reference_image_urls[0]`,941×1672,无单格裁剪图;父 task_35d250211981),技术本地复现单 `task_cbc9d07f1031`。**不是"没接"、不是模型、不是识别问题**。**技术本地已修**:VERIFY_SSL 关掉后能下载+识别1440×2560板+检测分隔线裁出首帧(~1.17MB);裁切失败改**安全失败**(不再兜底发整板);**P2 一并解**(编剧真收到 copyLanguage=en 等)。**SSL 技术已妥善处理(2026-07-13确认)**:`VERIFY_SSL=false` 确是全局开关(会碰商家外部参考图,我顾虑成立);已加生产保护=`APP_ENV=production` 强制开校验(false 在生产被忽略)+ 新增 `IMAGE_SOURCE_CAINFO_PATH`/`CURL_CA_BUNDLE` 走 CA bundle,倾向让 cos.lingkeai.vip 换公信证书而非按域名关校验。**⚠️残留风险=CA bundle 是成败点**:生产强制校验,若 cos 证书没修好+CA bundle 没配对→下载仍 result:19 失败→安全失败→商家拿不到视频。已提醒技术:部署后放真单前先单独验 worker 在生产能下载成功;最优先推 cos 换公信证书。**现状=还没上生产**(宝塔手动:传代码→配CA→重启PHP-FPM→重启worker→才验收,无ETA);本地过 php -l + 两个 smoke(v2_regression/scriptwriter_options)。**别测,等技术宝塔部署完+通知**。上线后烧一条核7点验收:copyLanguage=en进编剧/编剧lines英文/口播英文/i2v `image_url`≠`reference_image_urls[0]`(裁片进去了)/成片单场景多镜头硬切/画面无网格/任务详情有"六宫格首帧裁切"图。卡=`02_交付内容/给技术_视频2个P0_六宫格成片+编剧收不到选项_2026-07-13.md`。
2. **P2 selectedOptions 进编剧**(2026-07-12起):真实商家流程送进编剧的 `selectedOptions=[]`/`copyLanguage=""` 全空(实证 task_37575eff706d 高挑半身裙=明确选English那条 / task_a1415c741711 / task_3a836c411450),选英文出中文台词、人物默认亚洲脸。技术本地已连带修好(见 P1),**上生产同验**。运营侧已就绪(编剧 systemPrompt 认 copyLanguage 代码 en/zh_cn/ja)。
3. **待我做(等 P1/P2 上生产验通后)**:加"copyLanguage=en→首帧人物用西方/高加索人"规则(老板要英文视频配欧美脸;现默认亚洲脸,首帧也收不到语言)。
4. **压缩后老板2026-07-12指令**:核对后台总账(约46项/开放30)+前端清单(26条)还剩多少,能实测/GET核实已解决的划掉,产出干净待办。基准=现网实测(config用GET验,419只拦写、GET能读)。最新总账=`01_问题诊断/待办总表快照_2026-07-12.md`。
