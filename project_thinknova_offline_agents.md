...
5d. **✅C1一键发布定案(2026-07-14老板拍板)=P0扫码半自动**:成片页「发布」按钮→二维码→手机H5(保存素材/复制文案/打开App深链),零平台审核全平台通用;深链只承诺"打开App"不承诺直达发布页(scheme非官方会失效,做成运营可改配置);两坑=微信内置浏览器引导浮层+iOS用share sheet/长按。**P1=聚合商(Ayrshare类,一个API外包全部平台审核,月费几百刀)>自建直连(TikTok/Meta/YouTube三场审核)**,本期不做;国内平台(抖音要ICP备案+能力审批/小红书视频号无API)全走P0。文档=`02_交付内容/给技术_C1一键发布P0_扫码半自动_2026-07-14.md`。
5e. **🔴icon卡在前端白名单(2026-07-14实锤)**:商家前端bundle图标白名单仅~26键(sparkles/tag/receipt-text/layers/wrench/home/book/key/target等),**utensils/scissors/gem等行业专属键全不存在→config写了也渲染兜底**(这就是行业卡图标一直重复的根因,25行业vs~10可用键数学上无差异化)。**config侧已配齐**(两agent:25行业iconKey+场景卡4个优化flame/ticket/clapperboard/mic,回读落库)——已给技术29个lucide键清单让加进前端映射表+确认行业卡组件读industryFilters[].iconKey(旧规范里industryFilters本无icon字段),加完config即生效不用再动。
5f. **🔴lines定宽切片器bug(2026-07-15凌晨,已给技术文字)**:后端在模型输出不合规时兜底把整段文字按约18字定宽硬切进lines——断半句半词、还把指令文本切进台词(实证task_39ff35731f6b碎花裙"两件五折，马上|试穿看看"+末条混入"门店主体清晰可信"指令;英文单task_18f41bd2a6f5同构)。**这才是"台词僵、一句一句没连贯"的真凶**(触发看运气,水晶吊坠单没触发就正常)。要技术:去掉定宽切片,不合规改按标点边界切或重掷。**我侧已加(02:01 live)**:编剧systemPrompt【lines输出纪律】(恰好N条完整句、句句标点收尾、禁半句禁合并禁说明文字进lines)+【连贯铁律】(几句=一段连续话,上句带下句、可用而且/你看/关键是衔接、句式长短交错、至少一句问句感叹、禁同构排比禁句句名词开头)。
5g. **✅海报"运营指引上画面"已修(2026-07-15 02:09)**:老板会议海报3单出现"科技感视觉/干净舒适空间"角标=①卖点池指引整句("突出环境干净舒适、氛围放松。")被编剧照抄成角标②extraRequirement风格词(科技感蓝色主调)被画成文字(实证task_807042e57bb2)。修=blockTemplates.copy_rules.template加⑤卖点角标只写顾客视角2-6字短语、指引句必须转译⑥补充要求风格/颜色/构图描述只作画面处理不上画面。**海报编剧无config侧systemPrompt**(masterPipeline.scriptwriter只有textModel.modelId),文案规则全靠copy_rules。
5h. **✅新案例=自营销素材工厂(2026-07-15上线)**:`coworking_office_s11_product_pitch`「产品/服务·真人口播推荐」(商务办公·S11,CTA=dm_booking,预览暂用行业占位图——老板烧出满意片后设为真视频预览+记回填台账)。用途:平台自宣传视频(08号脚本"这人是AI做的")的镜1口播素材,台词写进补充要求(5句55字版已给老板);爆炸大字/录屏/25s拼接仍归剪映。定位资料包+新session提示词已交付(02_交付内容/给Codex_平台定位资料包_新加坡线下会议_2026-07-14.md;线下会议=每周1-2场75分钟体验工作坊,三币换算S$40/¥212入门)。
5i. **✅纯口播模式配方定型(2026-07-15 03:07实证,task_8825d5504f5e板完美)**:一人一景对镜连说的口播视频=**config三层+用户补充禁令四层齐压**才稳——①案例visualHint写"纯口播/六格同一人同一背景+画面绝不跟随台词" ②编剧systemPrompt【纯口播模式·优先级最高】(压过六拍骨架/跨档/微距无人格,+cells极简:第2格起只写"同景,仅手势/景别微变") ③i2v videoTemplate例外(纯口播板同人同景连说才正确) ④**决定性一脚=用户补充内容禁令**(deepseek对config条件分支时灵时不灵,03:03单四层缺④照样写滑手机/递手机):`全片只拍同一个人、同一背景对着镜头说话;画面禁止:操作平板、展示海报、屏幕特写、无人镜头、道具特写、场景切换;画面不跟台词走,台词提到啥也不许画啥。`(模型会"看台词配画面"=台词说到海报就画海报,必须明令画面不跟台词走)。**口播烧单模板=五句台词+这段禁令**(共~165字<200限)。字节:cells短写法后动态大降;静态已再削(vt 1840B/preset 1992B)。⏳待提技术:"常用补充"加「纯口播模式」快捷chip(客户不会自己写禁令)。案例`coworking_office_s11_product_pitch`预览图已借s11_owner(老板豁免)。4096爆单(02:44)已由字节手术+cells短写法解决(03:04单3995贴地过,短写法生效后宽裕)。
5j. **✅两条口播成片实测(2026-07-15 03:37,ffmpeg抽帧+whisper转写,成片已存我们公共OSS thinknova-previews/generated-assets/直下)**:两条台词都五句一字不差铺满15秒。**v2禁令版(task_b6829c0f52d5)**=纯口播成立(同背景/对镜/无杂画面)但**第一个硬切点换人**(0-3秒女生=首帧裁片,3秒后变男生到结尾;grok在切点弃首帧人物)——应急:掐前3秒即12秒完美男生口播。**v1无禁令版(task_04d8e6f775cb)**=人物全程一致但画面闹(悬浮ThinkNova大字+价格标签+手机UI=破零字幕)。**修(03:42落库)**:vt纯口播例外改"全片一镜到底不硬切+人物从头到尾=首帧那个人性别长相绝不变"——口播无切点=无换人风险。**待验:再烧一条口播看是否全程同一人**。视频分析法:成片URL在merchant任务详情(现存自家OSS无签名直下),curl下载→ffmpeg 1fps抽帧→faster-whisper转写→Read看帧。
