...
## 2026-09-08 模型路由(老板令:中文+参考图默认 H3,grok 也可)
- MiniMax 已充值;503 MiniMax-H3「实景还原版」已重新启用(`POST /admin/api/v1/models/503/toggle-enabled`)。
- 旧线 `businessUi.videoGeneration`:`modelAllowlistByDuration {10:[503,482],15:[503,482]}`,`defaultModelByDuration {10:503,15:503}`;工作台 `allowedVideoModelIds [503,482]`(H3 优先,kie 兜底)。
- 六宫格根因=482 kie「全能导演版·多镜头」在一条片里出多画面;415「口播优先版 15 秒」单镜头不切格。模型管理接口:`GET /admin/api/v1/models`(items 47 条),`toggle-enabled`/`set-default`。
- 黑场真值(09-08 复核):agent 级 `deliveryPostProcess.entranceBlackOverlay` 一直是 `{enabled:false}`(该层本就无效),案例级各整板案例是 `{enabled:true,seconds:0.5}`,没人关过;S11 口播场景与无 caseId 的任务本来就不套黑场,所以口播片开头拼板直接露出。headFreezeCover 是否上线已问技术(1.20)。

## 🔴🔴 503 首验(2026-09-09) —— `MiniMax-H3` 实景还原版

老板 09-08 令「用 503 模型去跑」。单:`task_1ac760362685`(15 秒,`ks_fs_shop_location` 开店选址,75 积分)。

**先确认派发**:子任务**实发 `modelId=503`**(不是只看 `requestedModels` 回读)。**没报 402** ——
09-05 那次 H3 余额熔断在这一单不复现。

| 项 | 结果 |
|---|---|
| 人声 | ✅ **有人声,mean -12.9dB** —— ⛔ 我担心的「H3 音轨被音乐占满」**这一单没发生** |
| 句间停顿 | ✅ 0.35 / 0.37 / 0.41 均匀 |
| max_volume | ⚠️ **0.0 dB = 削顶**(上一条 grok 单是 -2.7) |
| 片尾死寂 | ❌ **1.868s**(13.24→15.10),超对话片型 1.2s 判退线 |
| 画面文字 | ❌ **满屏乱码招牌**:「疾祷促」「痃裪促」/「ENEBIYEENOS LNHE」,是画面最大最红的元素 |

🔴🔴🔴 **根因是选题不是模型**:「开店选址」必须拍街边店铺 → 店铺必然有招牌 → 招牌必然出乱码。
我 extraRequirement 里写了「不要出现任何店名、招牌文字」,**但记忆里早写着"负面词挡不住模型给道具自带的字"**,
我明知这条还挑了一个必然出招牌的题材 —— **这是选题失误,不是模型失误。**

### 科普类视频的选题铁律(09-09 立)
**画面里天然不需要出现店铺、招牌、菜单、价目牌的题材才能做视频。**
- ✅ 可做:纯空间/物件类(家居布局、桌面物件摆放、收纳前后)——无文字、无人脸,双风险同时避开
- ⛔ 不可做:任何要拍店面/街景/菜单/价签的(选址、探店、价目)——文字必出乱码
- ⛔ 不可做:顾问说服型、带看型(老板 09-08 定,且 grok 溶解叠影实测) → [[feedback-boss-rulings]]
- 备选:**科普本来就适合静图**(三个法则=三张图),直连文生图 6 积分/张,可控得多

- 09-14 实测:503 H3 商家端显示名「实景还原版」/ 英文 True-to-life(admin 任务列表 model_name 就显示这个);482 grok 显示名「全能导演版」。旧线 defaultModelByDuration 现值 {10:503,15:503}(09-13 部署后)。

- 🔴🔴🔴 参考图**含主图**,名额顺序 人→景→产品;模型 id 500=MiniMax 字节上限低**必挂**(09-17 从索引下沉)。
