...
## 2026-07-02 决策：本机只拿数据，性能/规模改造等上正式服务器
用户定：**本机不再加负载**，先让采集器拿数据（SG 已采 1.7万+）；性能/规模问题**等上正式服务器用服务器跑**，别在本机打补丁。**最关键待服务器解决项**：`.data/platform-creators.json` 是单个 JSON 大文件、每次全量读写（采集器每 3.5s 回流一次）→ 几十万条会明显拖慢，服务器上迁**真数据库(SQLite/Postgres)增量 upsert**。其余见 `docs/BACKLOG.md` "性能/规模问题" 节（前端全量载入→服务端分页；采集器须在登录态浏览器跑的约束；头像缓存磁盘膨胀）。
注意区分：头像灰=代码bug已修(裸fetch加UA+Referer+超时)；达人库计数偏少=前端旧快照刷新即最新——**这俩不是负载问题**。
