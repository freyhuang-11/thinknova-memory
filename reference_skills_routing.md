...
### 🔴 质检线（原「评论线」会话）· 送验默认值（2026-09-20 起）

**出片和验片不能是同一个人。** 我出的片在上样片页 / YouTube / 投流之前，先过它。
**下一批直接发，⛔ 不用先问。** 它的常设默认（不用每次交代）：

- 零售 / 货架 / 包装类 → `fps=4 / scale=720` + 可疑处放大
- 判「有没有某一帧」→ **逐帧，不抽样**（8fps 找 1 帧事件必漏）
- 任何带阈值的判定 → **先问它有没有阳性样本**，没有就不当闸用

它只报「这一帧有什么」+ 严重度（带 n、注明是观测值），**能不能投由我判**。
⚠️ 它量的是**排版几何 / 像素事实**，⛔ **不是视觉观感** —— 「留白是构图还是残缺」这类它不背书，归我。

🔴 **我报的永远是线索，它量的才是结论。** 我今晚四次把「我改了 X」当成「只有 X 变了」
（`.ass` 写了≠画面上有 / 标题字没动≠标题变量没动 / 帧数时长没变≠没重烧 / 我改了 filter≠黑帧没了），
**四次都是它去量最终产物才发现的**。⇒ 送验时默认附「用什么方法、什么分辨率量的」。
...
- 🔴 **以上三条只管「CDP 端口」这条路,不等于浏览器自动化整体不通。**(09-19 总指挥指出)
  **claude-in-chrome 扩展不走调试端口**,用的是老板 Chrome 里现成的登录态,⇒ **不受 Chrome ≥136 那个限制**。
  ⇒ 再下「TikTok 自动化做不了」这种结论前,**先分清是哪条路不通**。
  ⚠️ 但**无人值守的定时任务照样不能靠它** —— 扩展要老板的 Chrome 开着且连着,固定点时刻不一定在。
  ⇒ 定时任务 ⛔ 不装 TikTok;要跑扩展这条路,09-25 第二批生产时找总指挥一起试。
...
8. 🔴🔴🔴 **⛔ 正文不许用 `keyboard.type`** —— 那个 contenteditable 是 React 控的,每敲一下重渲染、光标被拽回去:
   中文**掉字**(「你不用会写马来文,写你会的」→「你不用会来文,你会的」)、英文**乱序**(「You don't neOed to write」)。
   调 delay 治不了根。⇒ **`pg.keyboard.insert_text(正文)` 一次性塞**(CDP Input.insertText,不产生按键事件)。
   标签仍走 `keyboard.type`(要触发联想)。**打完一律回读比对,对不上重打,三次不过拒发。**
9. 🔴🔴 **TikTok 安全预检有「今日检查次数已达上限」**:一天传太多条就会超。
   超了之后点「预约发布」会**再弹一层**「继续发布?检查尚未完成,仍要发布?」〔取消〕〔**立即发布**〕。
   ⛔ **预约稿绝不点「立即发布」**(那四个字可能=马上发,违反老板「一天一条」)⇒ 按取消,**明天再跑这条**。
   ⚠️ 不处理这层弹窗 = 稿子卡住,日志上看着像「点了但没发出去」(09-17 06 那条就是)。
10. 🔴🔴 **上传页会生出几十个 web worker**,而 `connect_over_cdp` 会 attach **每一个 target**;
   实测 **54 个 target 时必连不上,清到 7 个立刻恢复**。⇒ 每条之间用 CDP HTTP `/json/close/<id>`
   **只关 `tiktokstudio/upload` 的标签**(⛔ 别碰老板自己的 WhatsApp 页)。脚本 `run_batch.py` 已内置。
...
### 🔴🔴 TikTok 自动发布 · 09-18 又买到的三条（都是我自己制造的坑）

1. **提交前必须等安全检查跑完。**
   点「预约发布」后会弹「**继续发布?检查尚未完成,仍要发布?**」〔取消〕〔立即发布〕,
   而预约稿⛔不能点「立即发布」⇒ 整条卡死。
   ⚠️ **我 09-17 把它归因成「今日检查次数已达上限」——错了**:09-18 新的一天、当天第一条照样弹。
   真因是**我点太快**(页面自己写着「正在检查。此过程大约需要 10 分钟」)。
   ⇒ 轮询到正文里**没有「正在检查」**再提交。实测等 10 秒就过了。

2. 🔴 **⛔ 别关掉最后一个标签页** —— Chrome 没有标签会**直接退出**。
   下一条任务报「连不上你的 Chrome」,看着像端口没开,**其实是浏览器没了**。
   同一个坑今天踩了两处(`run_batch.py` 清理 + `tt_post.py` 的 `finally: pg.close()`),都已加「先开空白页」。

3. 🔴 **作品列表是懒加载,只读首屏会把还在的稿子报成「没发出去」。**
   09-18 实测:7 条里 2 条被挤出首屏 ⇒ 误报失败。
   ⚠️ **这种假失败最危险 —— 会诱发重复发布**(小红书那 15 条垃圾草稿就是这么来的)。
   ⇒ 判「在不在」一律**滚到底**(`_verify7.py` / `_findone.py` 已内置)。
