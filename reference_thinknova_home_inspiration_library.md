...
metadata: 
  ...
  modified: 2026-09-13T14:08:48.732Z
---
...
- **坑**：任务列表 `page_size` 上限 50（用 100 只回一页）；创建接口慢（文生图秒回，图生视频带 URL 40–60 秒且会丢）→ ⚠ 技术 09-13:图生视频创建改为不带 Base64、图片物化移 worker(部署后复测耗时);09-13 实测 40–60 秒为修复前值(⚠ 09-13 技术文档口径,线上待复验)；供应商结果要手动 `POST /admin/api/v1/ai-tasks/{no}/refresh`（keepalive 投出即可）才落终态；排队约 30 分钟未派发 = `TASK_QUEUE_TIMEOUT` 判死退积分 → 图生视频待派发 ≥3 就别再投 → ⚠ 技术 09-13:排队硬超时≥2h 动态延长+自动重派 1 次(ai.task_queue_* 可调);「30 分钟判死/≥3 别投」为修复前值(09-13 22:40 线上实测:商家端 GET /api/v1/ai/tasks 列表项已带 queuePosition/estimatedWaitSeconds/queueReason,排队改造已部署;判死时长实际值待复验)；MiniMax 参考图最短边 ≥256px；gpt-image-2 会拒"女性手部特写"这类提示；页面驻留脚本 `00_规格与参考\灵感库烧片_页面驻留脚本_bootstrap_cycle_2026-09-13.js`（状态存 localStorage）。
- **停机点**：09-13 06:0x metaso H3 402 余额耗尽；kie grok 500（09-12 23:27 起）;技术 09-13:402 改为模型「暂不可用」+PROVIDER_BALANCE_EXHAUSTED,冷却到期自动恢复;余额是否已充现拉(⚠ 09-13 技术文档口径,线上待复验)。
