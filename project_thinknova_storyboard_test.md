...
metadata:
  ...
  modified: 2026-07-27T20:15:26.688Z
---
...
🔴 **样本量 n=1,不能反过来当定论。必须补烧 grok 扩样(建议≥4条成功)再定。**
**「黑屏不够黑」与「deliveryPostProcess 不执行」互相印证**:看到的开场压根不是我们配的黑场,是**模型自带的渐入**,所以是灰不是黑。→ 想要真黑、想要可控时长,只能靠后处理真跑起来(技术卡2)。
**下一步待办(接手先做)**:①补烧 grok ≥4 单扩样,逐帧量网格滞留 ②拿到那条成功单的 task_no 做联系表存证 ③`videoModelId=460` 疑点仍未核。
