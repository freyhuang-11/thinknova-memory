...
⚠️ **我一度猜"案例内容被巨量固定模板淹没(1.6%)"—— 算错了,当场撤回**:`mainPrompt` 共 **5681 字**,其中 boardCells 占 **1973 字 = 34.7%**。**信噪比理论不成立。**
...
### ⚠️ 先更正我自己两条错误结论
1. ❌ 我说过"这两条是同一个案例 `home_v2_S03_signature`" —— **错**。真值:
   - `task_b6074f858d21` = `home_v2_S03_signature`「招牌爆款产品」/ 场景 **S03**
   - `task_25047019b9d2` = `brand_product_s14_tvc`「大牌 TVC·高质感广告片」/ 场景 **S14**
   **两条是完全不同的案例、不同场景。** 所以"预设撞车(42条同参)"**不足以解释这一对**,那条解释对这一对作废。
2. ❌ 我说过"omni 没跟板走" —— 板层其实是有区分的(见下),问题不在跟不跟板。
...
- → **「案例只有150字」是我们自己没写满,不是系统限制。我先前提的技术卡是偷懒,已撤回。**
