...
- 09-08 起(部署后):同一件事多条规则打架由系统按来源优先级裁决(用户补充要求最高、运营模板次低),硬约束不可绕;运营模板里被硬约束覆盖的句子会在保存时报 `config_warnings`,看到就删,不要留「最高优先级」互搏。


## 2026-09-10 · 接口词清单会被机械套用;「一句一件事」会把台词切碎
真单 `task_ff254d6f1800`(S10,只填产品名):五句里三句开头挂「说白了/你想啊/反正」,句句碎片无主线。病症逐条对应提示词原句:「可带"反正/说实话"这种口语」+ skill 里的接口词清单 → 模型当填表;「每句至多一个逗号只讲一件事」+「每句必须含一件可核对的事实」→ 碎片 + 没料时编事实。工作台 `scriptwriterPrompt` 同样写着「多用口语接口词(说白了/你想啊)」。
**How to apply**:①给例词清单 = 模型逐句套,例词只能配「全片最多一次、只在第一句」这类位置限制;②「自然/口语」是形容词不可执行,与它同段的可执行硬规则(逗号数/每句含事实)会赢,要删的是后者;③对标结构写成顺序范式(惊讶→看到什么→感官细节→店名位置→叫人来)比十条禁令有效;④商家没填卖点时不许要求「每句含事实」。诊断档 `01_问题诊断\台词风格怪_根因在提示词不在模型_2026-09-10.md`。
- 🔴 **情绪/口语类规则必须带上限**（09-12 老板抓到：按摩片六句六个啦/呢/哟）。「至少两处语气词」+「陈述句最多两句」= 模型把每句都做成非陈述句+尾巴。写法：给区间不给下限（全片最多两处且不重复），并按板块分档（探店可两处，门店介绍/口播最多一处，公告零）。修正记录见 `00_规格与参考\ROLLBACK_四板块提示词改动_2026-09-11.md` §6。
- 🔴🔴 **09-12 起旧线 agent 保存有校验：`lineValidation` 任一语言在 `byLanguage` 与旁系副本不一致即 422，整份 config 都存不进去。** 副本有两份，第二份在 `stored_config.promptComposer.opsEditable.masterPipeline.scriptwriter.lineValidation`，**GET 返回的 `config` 视图看不到它**——必须读 `stored_config`、两份都对齐到 `byLanguage` 再 PUT `{config: stored_config}`。09-12 已按生效值(zh.15 = 60/80)对齐并保存，服务端归一化后只剩 `byLanguage`；B3b 至此落地，"要不要抬到 68"变成单字段决定。


## 2026-09-17 凌晨用实测钉死的四条
...
| 规则 | 原位置 | 挪到 | 效果 |
|---|---|---|---|
| 语言（非中文时零中文字符） | 54% | 2% | 中文残留 2 处 → 0 |
| 画外音（不许写正脸开口） | 92% | 14% | 正脸开口镜 → 0 |
| 台词长度 | 82% | 23% | 台词回到规定区间 |

**加规矩必须前置到 33% 以前。加在末尾等于没加**——这个错我一晚上犯了两次（cameraPrompt 那条加在 100%、不许重复那条加在 93%，都无效，挪前之后立刻生效）。

### 2. 工作台提示词上限是 3500 **字节**，不是 4000 字符
中文 1 字 = 3 字节。4000 字符的老规矩是 agent 线的，套到工作台会超 2.7 倍。
**改任何提示词前先 `new TextEncoder().encode(p).length` 量字节。**

### 3. 改镜头时长必须同时算台词的账
只把 minShotDuration 从 4 改成 2，没算"2 秒能不能念完一句话" → 合成直接失败：
```
SHOT_VOICE_TOO_LONG: 第 2 个镜头配音 2.99 秒，超过 2 秒镜头可安全调速范围
```
**定档口径**：3秒≤7英文词/12中文字、4秒≤9词/16字、5秒≤12词/20字。

### 4. 提示词里的字段名必须和当前契约完全一致
识图提示词里残留一处旧字段 `visibleFacts`，与新契约字段混用 → 模型输出新旧混版 →
`STUDIO_REFERENCE_ANALYSIS_RESULT_INVALID: Mixed recognition contract versions are not supported`
