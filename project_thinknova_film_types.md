...
metadata: 
  ...
  modified: 2026-07-27T15:52:49.739Z
---
...
### 🔴🔴 整板 vs 裁格 · 六宫格 · 黑屏 三者的真实关系(2026-07-27 深夜,反复多轮才理清)
**老板的原始设计意图(我一度删错东西才想明白)**:`storyboard_board`(整板当首帧)时,**开头必然是六宫格**——这是预期内的,`entranceBlackOverlay` 黑屏就是**专门用来盖住开头那一下**的;关键是模型要能**在黑屏结束前"走出网格"**变成单一全屏。老板说"之前做得到"。
**第一次尝试失败(23:33)**:只开 `storyboard_board`+黑屏,没给模型任何"离开网格"的指令 → **grok 整片全是六宫格(废片)**;**omni 报"图片读取时间太长"**(板图 1024×1536 太大)。
**修正(23:45)**:在 `stagePromptPresets.image_to_video.storyboardClarification` 顶部加【输入图是分镜参考板·不是画面内容】——明确说"这是3列×2行六格参考板,不是成片画面;**成片必须在开头第一秒内完全离开网格布局,立刻推近到第1格并铺满全屏**,之后按2-6格演进但每刻只有一个画面";`negativeGuard` 补"六宫格/分镜板/故事板/2x3网格/多画面并列/边框分割"。黑屏 0.5 秒(老板定,试过1秒觉得没必要)。
**⚠️ `i2vReferenceStrategy` 是全局开关,不能按模型分** —— 要么全裁格要么全整板。
**回滚方式**:`i2vReferenceStrategy` 改回 `panel_crop` 即可秒回滚(裁格模式下模型看不到板,不存在网格风险)。
**为什么想用整板**:裁格模式下 i2v 只把第1格当首帧、后面全靠文字自己演 → 成片和板差很远(实测 task_9054ba5a95dd:板图精致有手搓泡沫/水面涟漪,成片全没有还自己补了水泥厂房)。老板原话:"有区别没关系,但离得太远了"。

### 🔴 omni 台词限制(老板实测)
**omni 台词一多就念不清楚**(23:21:06 那条实证)。而 omni 只有10秒档、grok满血只有15秒档 → **按时长分档 = 按模型分档**,已利用这点:【纯口播·台词量按时长】**10秒(omni)只写40-48字**、12秒55-65、**15秒(grok)才85-95字**。
**口播案例卖点已全部清空**(60条S11):老板判断"口播不需要说卖点,台词会更好"——实测卖点(如selling_point_pro_service)会把台词带成"专业/规范/步骤"的空话。
