...
- 🔴🔴 [项目:故事板测试盘子(已出结论)](project_thinknova_storyboard_test.md) — **当前主线·07-28实烧10单**;🔴根因=`videoTemplate`那句「输入图片#第1镜首帧自然开场」是按裁格模式写的,整板模式下让模型拿整块网格开场(挂2.5秒);改法已备待放行PUT;**grok=中文口播/跟随80%/但5单挂2;omni=画面+英文/中文台词做不了/5单全成**;含admin CSRF从GET响应头直接拿+页面内抽帧联系表法
