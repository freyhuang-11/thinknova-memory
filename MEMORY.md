...
- 🟡 **多参无人声:EMMA 那条解了,我们这条没解(08-13)** — ✅EMMA 直连单 2 图:提示词开头加 **「不要背景音乐，只要人物讲话声音。」** 就出声(同剧本 A/B 实证);⛔「禁止静音」类**完全无效**。❌**我们管线 4 图单加了同款抑制句 + `sfx` 全改「无」后,`task_507d28e92a60` 实测仍然零人声** —— 所以「sfx 是我们的 BGM 对应物」这个推断**已被证伪**,音轨竞争不是我们失声的原因。**现存边界:1/2/3 图说话、4 图必哑,提示词层全压不住。** 剩下的路=问 kie 能否透传 `reference_audios`。⛔我当天三条错结论(≥2就哑/门槛在4/板排第一)全作废,病根=**单侧取样下规律** → [多参台账](reference_thinknova_multiref_model.md)
