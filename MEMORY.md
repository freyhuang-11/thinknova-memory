...
- 🔴🔴🔴 **多参无人声=真根因已锁定并修复(08-13,已上线)= 提示词里不许出现第二道「以…为准」硬锁** — 🔴**全片最多一道硬锁**,现留给人物(`以人物参考图为准`,脸锁得死);**场景/产品一律改成"参考"措辞**。实证:同为 4 图(人物+场景+产品),写「门店实景(**以场景参考图为准**)」→**零人声**;改成「门店实景(**陈设与光线参考场景图**)」→**全程有声,且场景还原度高**(柜台/陈列/窗景/连参考图里那个店员都复现)。完全不提场景→有声但只剩风格。对上 xAI「每张参考图锁一样东西」——**能锁多样,但不能用命令句同时钉死两样**。⛔ 沿途被证伪的六条:≥2就哑 / 门槛在4张 / 板必须排第一 / sfx是BGM对应物 / 黑边pad是元凶 / **人物图与场景图同时传就必哑**(F单 4图三锁齐上照样出声,此条作废) → [多参台账](reference_thinknova_multiref_model.md)
  ⚠️ **EMMA 直连单**那句「不要背景音乐，只要人物讲话声音。」对**她手写的 2 图单**有效(A/B实证),但对**我们管线 4 图单无效**,两条路的解法不同,别混用。
  🔴🔴 **08-13 深夜:我对编剧提示词的四刀已全量回滚**(systemPrompt 3999 / videoTemplate 414 / firstFrameTemplate 769,与动手前逐字一致)。原因=连改 5 次没单独验证,之后编剧开始连挂(重试 1→6 次,报错 `lines count does not match shot count` / `invalid JSON`),而同期技术也改了传图逻辑,**两边变量混在一起摘不干净**。回滚方式=**逐条反向替换**(存档文件回读 3995≠线上 3999,不可用于恢复)。**四刀里只有「商家成句台词一字不改」经过端到端验证,其余三刀价值未证。要重上必须一刀一验。**
- ✅ **技术已把 i2v 参考图上限砍到 2 张(08-13 22:07 实测生效)**:商家传 3 张、编剧仍收到 `referenceImageCount:3`,但 **i2v 实发只有 2 张**(此前同配置发 4 张)。⚠️ 同期出现**任务排队明显变慢**(pending 数分钟),是否与该改动有关未查。
- 🔴 **多参无人声·当前唯一可信结论**:**图越多越不稳** —— 受控实测(同案例同参数,判定口径=5句说满且无长断)**2图 2/3、3图 1/3、4图 0/3**;失败形态是**音轨被连续音乐占满或中途大段断**,不是静音。**提示词措辞全试过(硬锁/软锁/合并/不提/禁BGM/去音效)没有一个能救 4 图** → 解法方向=限制张数(技术已做)。
- 🟢 **商家成句台词不再被改写(08-13 已上线并端到端验证)** — systemPrompt【素材优先级】改成「**只把原话按逗号切成shotCount句,一字不增删,不受【开场】和末句字数约束**」;`task_8e5d7f2165f3` 成片实测:商家 77 字原话**一字未改未增未删**(旧版会砍掉「整理好自己」还自造一整句)。同批还删掉了写死的「七十六到七十九字」(**它和 10 秒单 40–48 的校验窗直接打架**,按时长注入的硬规则才是真值)。systemPrompt 3999→3979 字 → [编剧层](project_thinknova_0729_screenwriter_stack.md)
...
- 🔴 **08-13 未完待办** — ①✅**编剧层三刀已上线并回读验证**(整份 agent 的 PUT 一度被 Claude Code 权限分类器拦,老板放行后通过;**案例级 PUT 从来不受影响**)。回滚底稿在 scratchpad `ROLLBACK_sp_0813.txt`/`ROLLBACK_vt_0813.txt`;②周六 08-16 海报侧 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③技术卡 `OPS-POSTER-20260811-01` 可归档、**`OPS-POSTER-20260813-01` 待转发**;④公告栏上线后场景改名要发第一条公告;⑤⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案
