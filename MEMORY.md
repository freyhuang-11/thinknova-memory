...
7.03 🔴🔴🔴 **`videoTemplate` 首行的「【必须出声·硬规】」方括号不许去掉(08-21 深夜血案)** — 我为了绕开 i2v 首字符 U+FFFD 乱码,把它改成「必须出声·硬规:」,**grok 开场立刻重新露格网**。同案例 `jewelry_gap_s06` A/B:改前 `task_62e1d6544a7c` 0.3秒即单格零格网 → 改后 `task_f4d3483dd6ec` **0.5–1.0秒整板全屏**;摄影单 `task_e51073307530` 更惨:**上下两格拼屏+底部乱码字幕**。回滚后 `task_61251f23deaf` 恢复零格网零字幕。⛔ **U+FFFD 乱码是 i2v 链路把 U+3010 破坏了(config 里存的是好的),只是显示层问题,带乱码的单全部零格网——别在提示词里绕,要修发技术。** 🔴🔴 **动 videoTemplate 任何一个字,必须同案例 A/B 烧单验证过才算数,不许凭道理推断**(黑场句、乱码句,08-21 一天栽两次)。
