...
- 🔴 **08-16 晨待办池(视频线)** — ⓐ**等技术**:卡01三处落地→烧验收矩阵(zh多参/zh少参/ja少参/en多参各2单);卡02 A或B 落地→按选定案验收 ⓑ**等老板批**:minimax 吃不吃分镜板(1单)、成片自动抽非黑封面帧要不要并卡 ⓒ**ops 自做**:非中文语言语速(lineValidation 收紧,单刀烧验)、商家端上传引导文案(干净原图无水印无字,placeholder 可自改)、cropPriority×片型映射表(给卡02-B用) ⓓ日语探针 `task_3c0a410629ab` 编剧内容已验优(cells全英/日语台词纯净),成片链接548字节坏档未音验
