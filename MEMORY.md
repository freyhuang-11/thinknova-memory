...
- 🔴 **未完待办** — ②周六海报侧清单其余项 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③公告栏上线后场景改名要发第一条公告;④⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案;⑤🔴🔴 **黑场保留(08-16 普查定案,数据版)**:17 条 agent 原片(tempfile 供应商原片,叠黑场前)实测**首帧=板 1/17≈6%**(`caf8cd83` 帧0整板 0.2s 叠化)——低频但一露就是客诉,黑场必须留;**对话剧格网 0/6=panel_crop 后已治好**(老板"100%出格网"印象来自 08-12 前批次);"格网集中在 minimax"无样本可证(minimax 没吃过板,要验须专门烧)。正解仍是**平台自动抽 0.3–0.6s 非黑封面帧**(待并技术卡)。🔴取证方法学:**OSS 镜像=叠完黑场的后处理片,真原片只在 tempfile 域名(3天过期),历史取证要趁早**;直连线不加黑场
