...
6.4 🔴🔴🔴 **英文化已上线(08-16 凌晨,回滚=`00_规格与参考\ROLLBACK_全量英文化前_2026-08-16.json`)**:videoTemplate/firstFrameTemplate/durationPolicies(10/15/30)/负面数组(两层20项)全英文;systemPrompt **4000字整**加【cells语言】规则(cells 全英/台词 copyLanguage)。**验证单 `task_de951043a1fc`:cells 0 中文、台词纯中、compiledPlan `[Overview]` 拼装正常、除台词外残余中文=114字=恰好是技术三处**。⛔ **`opsEditable.stagePromptPresets.image_to_video` 五字段(prompt包装987字/negativeGuard/negativePrompt/storyboardGuard尾巴/clarification)运营写不进——两次 PUT code:0 回读原值=服务端丢写**,英文文本在 `02_交付内容\附件_OPS-VIDEO-20260816-01_stagePromptPresets英文文本.json`,归卡01 v2 技术三处之一(另两处=骨架标签、负面词第一段)。
6.5 🔴🔴🔴 **grok 提示词语言定律(老板 08-16 + 实测台账=`01_问题诊断\英文提示词实验台账_2026-08-15夜.md`)**:①**多参降级模式下混语言=音频宕机只剩BGM;整条单一英语(台词也英文写)=活**(混60字中文即哑/混日文死寂/纯英即说)②**少参(板+0~1)时中文骨架+任意语言台词历史全出声** → 真双变量=语言纯度×参考图数 ③**日语在多参下 0/5 判死**(令说/原文/罗马音/前置声明全灭,出声那次抗命说普通话)→ 非中英语言走少参老路或路由 minimax ④英文负面词生效=零字幕、三锁全中 ⑤纯英说中文=意译非原话字面 ⑥**带水印/带字的图绝不能当参考图尤其主图**(P6点评截图当主图→满屏字幕+点评水印)。技术卡 `OPS-VIDEO-20260815-01`+直测包已交;直连配方在台账。
...
- 🔴 **未完待办** — ②周六 08-16 海报侧 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③公告栏上线后场景改名要发第一条公告;④⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案;⑤🔴🔴 **黑场保留(08-16 普查定案,数据版)**:17 条 agent 原片(tempfile 供应商原片,叠黑场前)实测**首帧=板 1/17≈6%**(`caf8cd83` 帧0整板 0.2s 叠化)——低频但一露就是客诉,黑场必须留;**对话剧格网 0/6=panel_crop 后已治好**(老板"100%出格网"印象来自 08-12 前批次);"格网集中在 minimax"无样本可证(minimax 没吃过板,要验须专门烧)。正解仍是**平台自动抽 0.3–0.6s 非黑封面帧**(待并技术卡)。🔴取证方法学:**OSS 镜像=叠完黑场的后处理片,真原片只在 tempfile 域名(3天过期),历史取证要趁早**;直连线不加黑场
