...
- 🔴🔴 [东南亚语言扩充 08-04](project_thinknova_sea_languages.md) — **动输出语言第0步必读+当前第一优先待办**:老板早令=加泰语+全语言去β不测试/界面语言加SEA=技术i18n需求/grok多参P4-P6餐饮单待烧/vi第5单待查——**全部卡在 admin 登录态 401,老板登录后立即执行(文内有精确步骤)**;copyMode+海报多语言技术已修全验收过;视频五语4/5;两条硬教训(动模板必量最大单字节/字数窗别一次收太紧)
