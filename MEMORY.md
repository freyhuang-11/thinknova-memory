...
- 🔴🔴🔴 **多参无人声根因锁定(08-13 隔离实验 5/5)= 人物图与场景图同时传就没人声** — 与张数无关、与黑边无关、与音轨无关:`person`✅ / `scene`✅ / `person+product`✅ / **`person+scene`❌** / **`person+scene+product`❌**。对上 xAI 官方「每张参考图锁一样东西:一张脸、一个产品、一个地点」——**锁脸+锁地点同时提就切到不出声的通道**。**修法(要技术):商家同时传人物图和场景图时,不把场景图发给 i2v**(分镜板本来就是用这些参考图画的,场景已在板上,再传一遍纯重复)。⛔ 沿途被证伪的四条:≥2就哑 / 门槛在4张 / 分镜板必须排第一 / sfx是BGM对应物(4图单加抑制句+sfx全改「无」后**仍然零人声**);⛔EMMA 直连单那句「不要背景音乐，只要人物讲话声音。」对**她的2图单**有效(A/B实证),对**我们4图单无效**,别混用 → [多参台账](reference_thinknova_multiref_model.md)
