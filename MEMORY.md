...
- 🔴 **未完待办** — ①🔴🔴 **台词语气退化(老板 08-16 深夜:对话剧调优期最好,现在变差)**:头号嫌疑=08-16 凌晨为塞【cells语言】删的 4 段语气规则(朋友安利/情绪爆点/机器味句),次嫌=08-15 技术把编剧温度 0.7→0.6 → 排查案=`03_工作台\台词语气退化排查_2026-08-16.md`,等技术稳定后单刀 A/B 烧对照;②周六海报侧清单其余项 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③公告栏上线后场景改名要发第一条公告;④⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案;⑤🔴🔴🔴 **黑场关不得(08-22 实测定论,推翻我 08-21 的错误结论)** — 黑场**两层**:(a)`videoTemplate`「开场0.15秒纯黑」= 提示词层,是设计,不许删(见 7.02);(b)`deliveryPostProcess.entranceBlackOverlay` = 成片后处理遮罩,**开关在案例级**(`referenceCases[].deliveryPostProcess`,优先级 案例>Agent>系统,技术手册合订本 §4 早有记载)——⛔我 08-21 说的「服务端写死/必须发技术」**作废**,agent config 那四处配了没用是因为被案例层压着。⛔**但它关不得**:实测把案例关成 `enabled:false` 后烧单 `task_91c5834d1108`,**t=0.0–0.3 秒整屏六宫格拼板,0.6 秒才切进单格**(blackdetect 零黑场段)。**那 0.5 秒黑场的作用就是盖住这段拼板**,这也是 07-30「473 条整板案例开黑屏、114 条裁格案例无黑屏」的原因——整板要遮罩,裁格不用。已全量回滚(525/48/176 分布逐数对回)。要真去掉开场黑,只有三条路:改裁格模式(牺牲分镜)、技术在后处理裁掉开头 0.5 秒、或换能直接吃分镜板不露板的模型 → [回滚档](../../../../D:/SamsoData/Documents/视频制作平台分析/00_规格与参考/ROLLBACK_案例级黑场关闭_2026-08-22.md)
