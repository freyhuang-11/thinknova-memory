...
7.15 🔴🔴🔴 **编剧的提示词有两份,`outputContract` 最容易漏(08-24 血案)** — `promptComposer.screenwriter.staticTemplates.outputContract`(377字)整段拼进编剧 user prompt。两个坑都在这:①`line格式[[声线,感情,真人感:「台词」]]` → **模型照契约写,校验器不认(报 incomplete sentences),i2v 把外壳当台词念出来**;②「六格画面与动作需逐格推进」与「cells 必须恰好5项」打架 → 模型把「我准备怎么做」的说明当答案交(表现为 invalid JSON,响应仅60-130字符)。已分别改成「line只写纯台词本身」「各格画面」,**实测编剧从 5次5挂 → 1次过0挂**;🔴**「画面烧大字幕」的根因也是它**——外壳文本进了 cells 就被 i2v 当成「要画上去的字」,改完当晚 5 单(grok/H3 各半、有图无图、三个片型)**底部 30% 密采样 30 帧 0 字幕**,零文字硬规/负面词半年治不住的病一次断根。⛔**动编剧前必须同时看 systemPrompt + outputContract + 案例 visualHint 三处**,最稳是直接拉 `attempts[0].raw_request.prompt/.system` 看模型实收。
