...
- 🔴🔴 **08-13 待办(压缩后接着干)**:
  ①✅ **多参问题已收,根因查到**:不是"不按台词说",是**多图时一句人声都没有**;台词逐字进了提示词=组装层无辜。xAI 官方 `image`+`reference_images` 是非法组合→中转商(kie.ai)按图数自动切模式,**R2V 通道的声音要靠 `reference_audios` 驱动,我们从没传过**。⛔**别把它简化成"张数>N就哑"**(我 08-13 下过这结论当场被下一单打脸:老板手写单 2 张全哑、我们管线 2 张说话)。**老板 08-13 自己去问 kie.ai 客服**,四问已给(最值钱=能否透传 `reference_audios`)。答复回来前**不许动线上配置**。全部证据/矩阵/接口坑 → [多参模型台账](reference_thinknova_multiref_model.md)
