...
- 🔴🔴🔴 [语言包 08-16 晚半上线事故+回滚+当前真值](project_thinknova_language_pack_rollout.md) — **动 482/languagePacks/strict/验语言包修复前必读**:en 包顶掉 4000 字编剧规则(2P0+1P1 已进急件2 待发)、**482 agentInput 已整删=回 08-15 好状态**、多参被 `_merchant_multi_reference_enabled=false` 关着、自动封面帧已上线(旧提案ⓑ解)、模型行写入配方(PUT /models/{id})全在文内。ⓓ日语探针 `task_3c0a410629ab` 成片仍未音验
