...
- 🔴🔴 **08-15 技术已修四张卡 → 现在欠的是线上验收(旧记「待转发」作废)** — 08-14 合订本(`02_交付内容\给技术_合订本_四问题_2026-08-14.md`,5问题)发出当天技术回2份运营说明并部署:①**minimax截断→改拒单**(422010不建单不冻积分;上限=模型能力JSON `input.fields.prompt.maxBytes`,UTF-8字节,现1200,**调前必须先问上游真实上限**;⚠️列表接口看不到maxBytes,在编辑弹窗)②**编剧信封已剥+格式异常只重试1次**+新增 `masterPipeline.scriptwriter.textModel.maxOutputTokens`(**线上值是字符串"4096"**,temperature 0.6;⛔老路径 `screenwriter.textModel{temperature:0.7}` 仍并存,**动编剧模型参数认 masterPipeline 新路径**)③海报 label/promptText 已分离 ④copyMode full/minimal/none 已支持**但要运营自己配场景** ⑤copyLanguage 已进文案层(vi/id/ms 验过才能重开)。**验收欠账**:编剧验收单 `task_364bf8eb1c92` 在跑;海报③④⑤归周六。另:后台文章列表500已修、维护通知同一条只提示一次。
- 🔴 **未完待办** — ②周六 08-16 海报侧 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③公告栏上线后场景改名要发第一条公告;④⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案;⑤🔴🔴 **成片开头 0.5 秒黑场(`entranceBlackOverlay`)绝不能去(老板 08-14 定)**——i2v 的主图就是那张六宫格分镜板,**不挡住第一帧,成片第一页就是一张拼板图**。⛔ 我曾提议去掉,理由是「格网已用 `panel_crop` 解决」——**混淆了两件事**:`panel_crop` 治的是格线跑进画面,黑场挡的是首帧本身就是板。**代价照单接受**:成片首帧必为黑 → 插 PPT/社媒抓封面都会黑。**真正该做的是给每条成片配一张非黑封面图**(抽 0.3–0.6 秒后的帧),而不是动黑场
...
- 🔴🔴🔴 **硬切实验 08-14 · 一对 A/B 实测(实测·待核,每边仅1单)** — 案例 `food_beverage_s02_dialog_duo`,同参数同模型(482 `grok-imagine-video-1.5`/kie,15秒,无参考图)。**对照** `task_ad22ad97b37e` 景别跨度 2/1/1/2 → 硬切1处、最长叠化 **500ms**;**实验** `task_8d656c18aef8` 拉到全跨3档+焦点切换+不越轴 → **硬切0处、最长叠化 2583ms(更糟)**。**编剧层100%照做**(景别/机位/焦点全按指令写,source=text_model 无回退),**是 grok 自己不切** → 07-31「grok 做不到硬切=能力边界」的定论**暂时站得住**。案例已回滚(931字)。
  🔴 **但只测了半个配方**:那份资料的前提是「子格1必须纯黑屏=冻结锚,缺它整板被当一张图一起动、硬切失效」——**我们的板恰恰没有黑格**(shotCount=5+首帧格=六格全内容;我们的黑场是成片后期叠加的 `entranceBlackOverlay`,不在板里)。**「板内补黑格能不能救硬切」= 未测。**
  📏 **判读仪器(可复用)**:`scratchpad/cutdet2.py` —— ffmpeg 抽 64×114 灰度 rawvideo → numpy 逐帧平均绝对差 → 按「变化摊在几帧上」分硬切/叠化。⚠️ **别用 ffmpeg `scene_score` 判叠化**:叠化把变化摊到十几帧,每帧增量低于阈值会被整段滤掉,我 08-14 差点据此报出「grok 根本没切镜」的反结论。
