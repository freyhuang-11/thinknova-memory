...
7.15 🔴🔴🔴 **编剧提示词有两份,`outputContract` 最容易漏** — 「画面烧大字幕」和「编剧连挂」的根因都在它。⛔**动编剧前必须同时看 systemPrompt + outputContract + 案例 visualHint 三处**,最稳是拉 `attempts[0].raw_request` 看模型实收 → [详](feedback_prompt_change_hard_rules.md#outputcontract)
