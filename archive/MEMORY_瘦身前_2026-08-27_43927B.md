...
6.4 🔴🔴🔴 **英文化已上线(08-16)** — videoTemplate/firstFrameTemplate/durationPolicies/负面数组全英文;⛔`opsEditable.stagePromptPresets.image_to_video` 五字段运营写不进(服务端丢写)→ [详](project_thinknova_language_pack_rollout.md#英文化)
6.5 🔴🔴🔴 **grok 语言定律** — 多参混语言=音频宕机只剩BGM,整条单一语言才活;少参任意混语都出声且逐字照念;日语在 grok 多参下 0/5 判死;⛔带水印/带字的图绝不能当参考图 → [详](project_thinknova_language_pack_rollout.md#grok语言定律)
...
7.02 🔴🔴🔴 **`videoTemplate`「开场0.15秒纯黑」是设计不许删** — 它盖住拼板出现那一瞬;删了 grok 开场露六宫格 1.5 秒(A/B 已证)。⛔删提示词任何一句前先问「它为什么在这里」;⛔动 videoTemplate 必须先存回滚文件 → [详](feedback_prompt_change_hard_rules.md#vt-黑场句)
7.03 🔴🔴🔴 **`videoTemplate` 首行方括号【】不许去掉** — 去掉 grok 立刻重新露格网(同案例 A/B 已证)。🔴**08-25:i2v 实发串的 `【` 一直在丢(修复后变成静默删除),但实测不露格网——「整对删掉」与「只丢左括号」不等价,别套用本条下结论**。🔴🔴**动 videoTemplate 任何一个字必须同案例 A/B 烧单验过才算数,不许凭道理推断** → [详](feedback_prompt_change_hard_rules.md#vt-方括号)
7.05 🔴🔴🔴 **生图链也被截断(08-21 夜上限提到 7000,但原文 12000+ 仍被从尾部硬切)** — 所以**关键规则必须前置进 `taskGoal.firstFrame` 安全区**,前置的一条都不能撤。判据字段 `input._prompt_truncated` → [详](feedback_prompt_change_hard_rules.md#生图链截断)
7.15 🔴🔴🔴 **编剧提示词有两份,`outputContract` 最容易漏** — 「编剧连挂」的根因在它;⚠️**「画面烧大字幕」只是被它压低了、没断根**——去掉 `line` 外壳后 31 单里 30 单干净,但 `task_1686c9f1ff76`(grok,5次尝试)末尾仍烧出两行乱字幕,来源查实**只在 `line` 里**(visual/boardCells 都没有),即 **grok 会自发把台词渲染成字幕**,提示词层挡不住。🔴**老板 08-26 定:偶发(约 30:1)不处理**——剥掉 line 会伤口型和语速,拿确定风险换不确定收益。⛔我说过的「字幕已断根」**作废**。⛔**动编剧前必须同时看 systemPrompt + outputContract + 案例 visualHint 三处**,最稳是拉 `attempts[0].raw_request` 看模型实收 → [详](feedback_prompt_change_hard_rules.md#outputcontract)
7.17 🔴🔴🔴 **验收前先确认「这个指标在修复前是什么值」** — 08-25 我拿「生图截断到 6999 字节=回退字符边界=修复生效」当证据,子agent扫 100 单发现**修复前就有 26 次 6998/6999**,这行为一直就有;我还拿了一个**根本没有 i2v 子任务**的单去验 i2v 首字符。⛔没有前值基线的指标不能当验收证据。
7.18 🔴🔴🔴 **不许拿数值检测器代替人眼看图**(08-25 一天栽两次) — ①用字幕检测用的**底部32%裁切图**去判断画面内容,误判「S06 是皮肤微距片」,拉全画幅才发现内容是好的;②自建「底部边缘密度」筛字幕,**把已人眼确认零字幕的 B1 标成可疑、把真出字幕的 G2 标成干净**。⛔联系表必须人眼逐格看。
7.195 🔴🔴🔴 **编剧「疯狂失败」有一半不是提示词的锅(08-26 实测 118 单/241 次调用)** — 回退链 `luna → deepseek-v4-pro → terra×2 → flash`;**19 次超时 100% 是 deepseek-v4-pro(每次 180s 收 0 字节)、32 次「非 JSON」100% 是 gpt-5-6-terra(返回 ~100 字符的「我会…生成合法 JSON」预告帧,带 progress:100)**;两个通道成功率 13%/22%,一条挂到底的单 6.8 分钟里 5.4 分钟纯白等。⛔**下"编剧失败=提示词写坏了"的结论前,先按模型拆 `__attempt_trace`**;主模型 luna 自身 64%(内容类失败才归我)。技术单 `OPS-VIDEO-20260826-01`
7.196 🔴🔴 **负面词挡不住生图编造(08-26 实证)** — beauty 负面词里**早就有「滴管/改变瓶型/增加瓶身部件/未提及的道具」,滴管照样出现**,还编出注射器抽液(参考图无、提示词全链零处提及、**首帧板就有=生图阶段编的**)。⛔别再往负面词堆同类词,要走正面指派。另:**行业专属负面词只进 `videoStrategyNegativePrompt`,没进 `imageToVideoNegativePrompt`**
7.19 🔴🔴 **`lineValidation` 校验的是剥掉 `[[声线:「」]]` 外壳后的字数**,不是 `line` 原始长度(样本最大虚高 121 字)。核字数前先剥壳。
...
- 🔴🔴 **08-15 技术已修五问题,欠线上验收**(minimax截断改拒单 / 编剧信封已剥 / 海报 label-promptText 分离 / copyMode 三档 / copyLanguage 进文案层)。🔴**动编剧参数认 `masterPipeline` 新路径,老路径 `screenwriter.textModel` 仍并存** → [总账](../../../../D:/SamsoData/Documents/视频制作平台分析/03_工作台/待办总账_从记忆迁出_2026-08-24.md)
...
- 🔴 **未完待办清单**(台词语气退化排查 / 周六海报侧清单 / 公告栏改名 / 6条 prefill.offer 是说明文案待换 / 黑场关不得)→ [总账](../../../../D:/SamsoData/Documents/视频制作平台分析/03_工作台/待办总账_从记忆迁出_2026-08-24.md)
...
- 🔴 **待测小池**(编剧反呆板三刀草案 / 门头做进核心设置 / 10秒单走15秒 / video-shotcraft 只做演示片)。🔴**风格类改动一律「单刀→AB烧验→老板过目→再下一刀」** → [总账](../../../../D:/SamsoData/Documents/视频制作平台分析/03_工作台/待办总账_从记忆迁出_2026-08-24.md)
...
# 商务与融资
- 🔴🔴🔴 [海外邮件营销线](project_overseas_email_outreach.md) — **我主理的新马英文商户冷邮件线(08-24建成跑起来)**;系统在`03_工作台\邮件推广系统\`,全自动每天10点发/22点日报;名单Overture免费27万邮箱;判定器移植5173;挂总指挥体系走vault信箱对接;❗正则/中文代码绝不走heredoc、CSV空值坑、13个信息需求等总指挥回真值
