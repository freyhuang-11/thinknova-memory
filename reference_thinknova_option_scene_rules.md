...
metadata: 
  ...
  modified: 2026-09-16T07:01:45.748Z
---

# 场景提示词 / 选项规则 / 案例写入(2026-07-29 实证 · 08-12 大改)

## 🔴🔴🔴 2026-09-16 实拉线上真值(覆盖下面「10 个」那节,报数前先看这里)
命令:在 `api.thinknova.top/robots.txt` 轻页跑 `fetch('/api/v1/business-video-assets/config',{credentials:'include',headers:{'x-thinknova-locale':'zh'}})`。
🔴 **视频线和海报线现在是两套分类,别混着报**:

| | 行业 | 场景/用途(`businessActions`) |
|---|---|---|
| **视频线** `business-video-assets/config` | **全部 + 10 大类**(前台选择页另有「其他行业」⇒ 屏幕上 11 块):餐饮小吃/特产·生鲜·礼盒/自有品牌产品/美容美发美甲/养生·健康/零售店/家居·装修·绿植/上门与到店服务/玩乐·住宿/教培·顾问 | **7 个**:拍产品·一眼心动 / 有活动·现在就来 / 像达人来探店 / 介绍我的店 / 前后对比·眼见为实 / 真人开口说 / 广告大片·不用说话 |
| **海报线** `business-assets/config` | **全部 + 21 个旧行业**(餐饮茶饮/美业服务/花店绿植/中医理疗/金店珠宝…**没动过**) | **19 种**:新品上新…网页首图/商家名片图/发给客人的商品图 |

- ⇒ **「场景表 10 个」已作废**(那是 08-12 的视频线数值);⛔ 旧场景名「手艺揭秘」「商品介绍」在视频线**已不存在**。
- 视频线 `detailOptionGroups` 9 组:输出语言/投放平台/视频比例/视频风格/出镜方式/画面重点/结尾引导/节奏强度/门店位置强调;另有 `quickRequirementChips` 9 条、`sellingPointOptions` 32 条。
- **4 个前台 agent 中英名(`GET /api/v1/agents`)**:爆款复刻=Copy a Hit Video / 海报制作=Make a Poster / 视频制作=Make a Video / 长视频制作=Make a Long Video。
- 🔴 **教训**:2026-09-16 我拿老板一张**视频线**的截图,去改提词器里讲**海报线**那一步的数字(21→11),
  把对的改成了错的。**同一个词(行业/场景)在两条线上是两套值,改之前先认清这句话讲的是哪条线。**
...
- 🔴🔴🔴 09-11 **案例接口 pageSize 服务端封顶 60**;传 200 只回 60 条,`data.pagination.total` 才是真数。真值:**视频 749(启用 611)、海报 1091(启用 996)**。09-10 那份「全库 60 条」审查作废。
- 09-11 全库真值:视频 10 场景 / 27 行业 + 8 条无行业;海报 19 场景 / 27 行业。案例说明中位长度:视频 334 字、**海报仅 51 字且 0 条有【画面】【文案方向】结构段**。
- 🔴 **情绪真值(只数正向指令,09-11 修正)**:全库 749 条只有 **149 条(20%)** 写了情绪指令,**546 条压根没提**,54 条明写不要。分场景:S08 100%、S11 52%、S05 29%、S01 25%、S06 17%、S10 10%、S02 9%、S07 5%、S04 3%(另 54 条明写「不写情绪」= 正确,与总纲一致)、S14 0%。⚠️ 09-10 我用关键词扫「情绪」把否定句也算成有情绪,得出「S04 86% 反了」= **误判,已更正**;老板 09-11 定「信息公告不需要情绪」,现状本来就对。**「没提情绪」才是真问题:模型自己挑了沉稳/平静。**
- 09-11 其他计数:说明写死「五格」349 条、shotCount=5 726 条、缺【台词方向】114 条、带沉稳/平静/沉着 68 条、要求对镜说话 42 条、禁用 138 条、过短 16 条、网红用词 6 条。
- 📄 报告(artifact):全库扫描与优化方向 https://claude.ai/code/artifact/ (见会话);本地 `scratchpadudit-0911.html`。

- 🔴🔴🔴 09-11 老板定的**四板块**(取代按场景思考):营销视频(S01/S02/S07/S14/S08,434 条 58%)、口播视频(S11/S10,126 条 17%)、门店介绍视频(S04/S06,111 条 15%)、探店视频(S05,70 条 9%)。每板块一套默认:情绪档/出镜/镜头职责/节奏/拍摄方式。**规则上移到板块层,案例层只留差异点、不再往里加新规则。**
- 09-11 老板定:**海报层主要是画面**,不套四板块;海报案例说明本来就是版式构图导向(「左右两格对比版式,同机位同构图同光线」),09-10 我说它「缺台词方向段」是拿视频标准套海报,**误判已更正**。海报 19 场景里 S14–S19 六个是「图片用途」非「内容方向」。
- 09-11 撤回「加拍摄方式轴」提案:四板块已吸收(探店=运动跟拍/口播=真人口播/门店介绍=桌拍微距/营销=智能匹配),再加下拉重复且违背零动脑。
- 🔴 **待办优先级**:①有案例 vs 无案例 四单对照(决定案例层命运,其余都等它)②旧线总纲 10 段场景要点压成 4 段板块要点(腾约 300 字写情绪弧)③工作台补三个板块台词规则(画面等 T6)④清库存(视频禁用 138/无行业 8/过短 16,海报禁用 95)⑤查 15 秒字数窗真实注入来源。

- 🔴🔴🔴 任何列表接口先读 `pagination.total` 再翻完;⚠️**两个案例端点的 pageSize 上限不一样(记忆里 20/60 两说)——报数前现拉**;终止条件用 `items.length===0`(09-17 从索引下沉)。
