...
7.16 🔴🔴 **加规则前先问「模型能用什么方式满足它而不解决问题」** — 08-24 我查到「67%打回是缺句末标点」就加了「每句必须以。结尾」,**模型改成给每句套 `[[女声,平静,真人感:「…」]]` 外壳**,句号包进内层,失败率反而 4次3挂→5次5挂。⛔ 统计相关性 ≠ 因果 ≠ 加禁令能修。同夜加的三条(强制句号/禁外壳/每句≤16字)**全部撤回**;其中「每句≤16字」被数据直接证伪:**成功样例 18 句≥17字(最长32字)、12句带2逗号,校验器根本不查字数和逗号**。
...
- 🔴 **未完待办** — ①🔴🔴 **台词语气退化(老板 08-16 深夜:对话剧调优期最好,现在变差)**:头号嫌疑=08-16 凌晨为塞【cells语言】删的 4 段语气规则(朋友安利/情绪爆点/机器味句),次嫌=08-15 技术把编剧温度 0.7→0.6 → 排查案=`03_工作台\台词语气退化排查_2026-08-16.md`,等技术稳定后单刀 A/B 烧对照;②周六海报侧清单其余项 → `03_工作台\周六待处理_海报侧问题清单_2026-08-16.md`;③公告栏上线后场景改名要发第一条公告;④⚠️ 6 条案例 `prefill.offer` 写的是说明文案,烧样图前必须换真实文案;⑤🔴🔴🔴 **黑场关不得(08-22 实测定论,推翻我 08-21 的错误结论)** — 黑场**两层**:(a)`videoTemplate`「开场0.15秒纯黑」= 提示词层,是设计,不许删(见 7.02);(b)`deliveryPostProcess.entranceBlackOverlay` = 成片后处理遮罩,**开关只有案例级**(`referenceCases[].deliveryPostProcess`)——🔴🔴**技术 08-24 代码确认:该字段被明确限制为案例级,Agent 层写了会被清除**;⛔旧记的「优先级 案例>Agent>系统/合订本§4三层覆盖」对这个字段**不成立,作废**;⛔我 08-21 说的「服务端写死/必须发技术」也作废,agent config 那四处配了没用就是因为 Agent 层被清掉。⛔**但它关不得**:实测把案例关成 `enabled:false` 后烧单 `task_91c5834d1108`,**t=0.0–0.3 秒整屏六宫格拼板,0.6 秒才切进单格**(blackdetect 零黑场段)。**那 0.5 秒黑场的作用就是盖住这段拼板**,这也是 07-30「473 条整板案例开黑屏、114 条裁格案例无黑屏」的原因——整板要遮罩,裁格不用。已全量回滚(525/48/176 分布逐数对回)。🔴🔴🔴**正解=盖不是切(08-24 定,已提技术单 `OPS-VIDEO-20260824-01`,技术已确认根因并同意做 `headFreezeCover`,层级走 案例>Agent>系统、缺省 `{enabled:true,mode:"auto"}`)**:用第一个亮度稳定帧覆盖开头黑场段,音轨时长一字不动。⛔**「裁掉开头 0.5 秒」这条路已被实测否掉**——人声比画面早(实测 `task_86f3bb50b353` 人声 0.125s vs 画面稳定 0.417s;`task_564006dcd1fd` 0.225s vs 0.458s),裁开头直接吃台词。⛔微信/抖音只读 MP4 第 0 帧,不读 `thumbnail_url`,所以平台内封面正常≠分享出去正常 → [回滚档](../../../../D:/SamsoData/Documents/视频制作平台分析/00_规格与参考/ROLLBACK_案例级黑场关闭_2026-08-22.md)
