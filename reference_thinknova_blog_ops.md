...
## 🔴🔴🔴 09-11 我亲自踩的两个坑(代价:六篇封面被洗掉 + 定时没生效)
...
一次没带封面字段的 PUT,把六篇的封面全洗成空,而封面是发布硬门槛。
🔴 **规矩:PUT 前先 GET 整条,把要保留的字段全部带上**(标签、封面、SEO、published_at…)。
　 这跟 08-23「案例表 321 条被覆盖」是同一条铁律的不同表现面。

## 2026-09-18 实测:排期未到的文章对外是「200 + 空壳」

- 已到点的 63 篇:SSR 正常,`<title>`=文章标题,h2 3~6 个。**博客 SEO 结构没问题。**
- 🔴 **未来排期的 10 篇:HTTP 200,但 `<title>` 是「ThinkNova」、h2=0、正文不渲染。**
  ⇒ 爬虫看到的是**一批标题完全相同的空页**。
  ⚠️ **这很可能就是 GSC 里「重复规范 15 页」的来源**(数量级对得上),
  但我**没有拿 GSC 的 URL 清单逐条比对过**,所以是**强假设不是定论**。
- **我这一侧能做的**:少提前排期,尽量当天建当天发 ⇒ 空壳页停留时间最短。
- **真要根治**才是技术单:排期未到的文章对外应 404 或 noindex,不该返回 200 空壳。
  ⛔ 按 L8.791,开单前先把上面那条自查做完。

### ✅ 同日:75 篇全部补上了 CTA
- 09-18 之前**线上 75 篇正文里一个产品链接都没有** —— 读者看完无路可走(转化缺口,不只是埋点缺口)。
- 已全部补:`https://thinknova.top/?src=blog&p=<slug>` + WhatsApp `+65 9868 5036`(两条路:自己试 / 让人带)。
- 🔴 **PUT 的坑**:`GET` 回来的是 `category`{obj} / `tags`[obj],但 **PUT 要 `category_code` / `tag_codes`**,
  直接塞回去 → 422「category_code must use lowercase...」。且 PUT=整体覆盖,改完必须逐字段比对回读。

## 2026-09-18 GSC 趋势:老板说「最近 seo 不行了」—— 核实,他是对的,但真相更糟
...
- ✅ **确认在掉**:展示 43 → 27,**连跌三周**。
- 🔴 **但真问题不是"掉"**:最近 28 天 **非品牌词 = 0 个查询、0 展示、0 点击**。
  只有 3 个品牌词(thinknova / think nova / scriptnova)。**63 篇已发布文章的自然搜索产出 = 0。**
- ⚠️ **「平均排名 17 → 5」是假象**:不是变好,是**长尾全掉光只剩首页品牌词**。⛔ 别拿它当好消息报。
- 🔴 **根因是选词不是技术**(和 09-15 诊断一致):26 个页面排进前 10 但展示 ≤5 ⇒ **排得上去,但那些词没人搜**。
  ⇒ **用同一套选词再写 63 篇,结果一样。**
- ⚠️ **`:443` 坏 URL 还在**(`/vi/blog/...`、`/vi/help`,各 1 展示)。
  09-15 我判断「旧索引残留会自行消化」—— **三天后仍在,那个判断下早了**。
- 💡 **拿真实用户语言代替拍脑袋选词**:`邮件推广系统/outreach.db` 的 replies 里有真实店主自己的措辞;
  TikTok/YouTube 评论同理。⇒ 选词应该来自客户怎么说,不是我们怎么编。

## 2026-09-18 · 选词改成「真实搜索信号」(老板:「你去处理!博客不停」)

**新选词流程**(脚本 `博客发布/_kw.py`):
1. 种子词 → **Google 搜索建议**(`suggestqueries.google.com`,按真实热度排序,免费)
2. 字母扩展 + 多地区多语言(sg/my × en/ms/zh)
3. ⛔ 过滤求职/代理商类词(jobs / agency / salary / course)——那些人不是买家
4. 和已发 slug 比对 → 找「有人搜、我们没写」的空白
5. 🔴 **人眼再筛一遍**:机器筛不掉**品牌名**(Hey Cafe / Setia / Popular)、
   不相关意图(学校、找摊位、要 PDF)、已退出市场的平台(Zomato 在新马已退)

⚠️ **这份数据的边界**:只说「有人搜」,**不说搜了多少**。⛔ 不许当搜索量报给老板。

**人群锁定(实测,不是拍脑袋)**:冷邮件 8758 封按行业拆 →
**只有餐饮有回应**(1766 封 → 9 真人回 → 4 有兴趣);美业+汽修+创意服务+健身 **2186 封 0 回**。
⇒ 选题锁餐饮/小吃店,⛔ 不再一篇一个行业地摊。

**第八批五篇已排**(`_add8.py` / `_pub8.py`):09-21 / 09-28 / 10-05 / 10-06 / 10-07,一天一篇。
🔴 **这是验证批,⛔ 不铺开**。2–4 周后跑 `YouTube/gsc_trend.py` 看这 5 个词有没有进非品牌词:
 · 有展示 ⇒ 方法对,再铺 10 篇
 · 零展示 ⇒ 这套方法也不行,得承认 SEO 在我们的时间尺度上不产出,产能挪走
**⛔ 不复盘的选词 = 占星。**

**顺带**:`_pub8.py` 会自动找**没被占用的工作日**插队,不用手排。
`build()` 必填 18 个字段,漏 `summary_zh/en` 会 KeyError(09-18 踩过)。
