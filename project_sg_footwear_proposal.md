...
**🔴 ERP/CRM 已全部清除**(客户已单独申请过 ERP/CRM 系统，补贴不批重复项)：报价 M1-03 由「CRM/ERP/官网数据集成」改名为「数据集成与统一数据层」(功能与 6,000 保留、总价不变)；其余所有 ERP/CRM 字样中性化为「客户现有系统」，架构图/数据流图节点 ERP/CRM→「现有业务系统/现有会员系统」。四份文件+PDF 已核对 0 处 ERP/CRM。以后凡涉及这份提案，不许再出现 ERP/CRM 字眼。
