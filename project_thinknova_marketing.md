...
metadata:
  ...
  modified: 2026-08-02T16:38:38.229Z
---
...
✅ **成品已做**:`deck.html`(08-03 老板要求"设计一下排版调整一下")。一个文件,双击就放。
- **数据驱动**:`AUD` 对象换听众(菜单选,全场文字自动换);`tier` 换时长档(键盘 1/2/3 = 15/30/75 分钟,不在档的页自动跳过)。**这两个是"万能"的实现方式。**
- 键位:← → 翻页 · **N 讲稿**(每页的口播词和我的提醒都在里面,投影不显示)· O 总览 · F 全屏 · `#9` 直达第9页(`#3t75a-ds` = 直销包75分钟档第3页)
- 设计:牛皮纸底 + 楷体大字 + 分段配色(开场橙/祛魅墨绿/产品蓝/证据砖红/收尾金),照 hallmark 反 AI 味那套做的。**结构不重样**——陈述页/四宫格/左右对比/表格/案例墙/大数字各有各的版式。
- **案例已放进去**:`cases/01-04.mp4`,四个不同行业全部 15 秒真成片。`qr.png` 放同目录就自动显示,没放会提示。
- 自验:无头 Chrome 抽了封面/痛点/证据/价格/福利五页截图,`_核验/` 下。
