...
metadata:
  ...
  modified: 2026-08-12T17:51:01.637Z
---

# 海报案例库视频语言清毒(2026-08-09,老板发现"海报提示词里有视频内容")

## ✅🔴🔴🔴 2026-08-12 技术已修:海报可见文案来源在服务端收口(CTA 编造事故彻底闭环)
技术文档《运营说明_海报预置文案与提示词守卫修复_2026-08-12》,已归档 `00_规格与参考\技术文档归档\`。四条:
1. **CTA 只按用户实际选择生效** —— 没选或选「不指定」就不再自动补"到店购买/私信预订/扫码下单/团购咨询"。
2. **卖点只按用户实际选择生效** —— 不再拿场景标签、行业标签、案例标签自动补成卖点。
3. **案例/场景/行业名不再是可见文字来源** —— 它们继续用于画面理解与视觉约束,但不能当标题、副标题、卖点、CTA;商品名缺失时标题也不再回退用案例标题。
4. **异步编剧加服务端二次守卫** —— 模型即便异常返回 CTA/卖点,合并结果和最终生图提示词仍会清空;最终提示词新增"内部场景/行业/案例信息不得渲染为画面文字"。
→ **运营侧不用重填任何配置**;想要 CTA/卖点就在前台明确勾选。
→ ⚠️ **技术只跑了 PHP 语法检查和 smoke 回归,没做真实出片验收**。文档要求发布后按三条口径**各抽样至少 5 单**:①不选 CTA 连续出图不应出现默认引导 ②不选卖点不应出现案例名/行业名/场景名拼成的卖点 ③明确选了 CTA 与卖点仍能正常进文案。**这批验收还没做。**
→ 我方那张 `OPS-POSTER-20260811-01`(服务端预置 copyPlan)**可以归档了,技术已实现**。
...
- 🔴🔴 海报两条结构性事实:`visualHint` **送不到文案模型**;尾部「海报标题:」串=**服务端硬编码**(09-17 从索引下沉)。
